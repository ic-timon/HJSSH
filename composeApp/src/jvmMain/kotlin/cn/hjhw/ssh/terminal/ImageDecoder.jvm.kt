                ...
                when (format) {
                    TransmissionFormat.RGB -> {
                        ...
                        if (pixelBytes.size < width * height * 3) {
                            println(
                                "      [ImageDecoder] Insufficient data for RGB: expected ${width * height * 3}, got ${pixelBytes.size}",
                            )
                            return null
                        }
                    ...
                    TransmissionFormat.RGBA -> {
                        ...
                        if (pixelBytes.size < width * height * 4) {
                            println(
                                "      [ImageDecoder] Insufficient data for RGBA: expected ${width * height * 4}, got ${pixelBytes.size}",
                            )
                            return null
                        }
