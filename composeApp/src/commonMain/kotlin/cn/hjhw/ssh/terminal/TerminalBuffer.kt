...
class TerminalBuffer(
        ...
        try {
            if (isMore) {
                ...
                if (accumulator != null) {
                    ...
                    println(
                        "      [TerminalBuffer] Multi-chunk complete, totalLength=${fullData.length}, size=${finalWidth}x$finalHeight, U=$finalUnicodePlaceholder",
                    )
                } else {
                    // 单块传输
                    fullData = data
     ...
     */
    private fun writePlaceholdersWithDimensions(
        startX: Int,
        startY: Int,
        columns: Int,
        rows: Int,
        ...
        println(
            "      [TerminalBuffer] Writing placeholders at ($startX,$startY), size ${actualColumns}x$actualRows (pixel: ${pixelWidth}x$pixelHeight)",
        )
